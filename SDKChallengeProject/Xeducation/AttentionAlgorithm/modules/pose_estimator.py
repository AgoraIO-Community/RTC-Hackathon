...
class PoseEstimator:
    ...
    def get_faced_line_length(self, img, R, t):
        points = np.float32(
            [[80, 0, 0], [0, 80, 0], [0, 0, 80], [0, 0, 0]]).reshape(-1, 3)

        axisPoints, _ = cv2.projectPoints(
            points, R, t, self.camera_matrix, self.dist_coeefs)

        font = cv2.FONT_HERSHEY_SIMPLEX
        red_line_length = sqrt((axisPoints[3].ravel()[1] - axisPoints[2].ravel()[1]) ** 2 + (axisPoints[3].ravel()[0] - axisPoints[2].ravel()[0]) ** 2)
        cv2.putText(img, str(red_line_length), (20, 20), font, 0.5, (0, 0, 0), 1)
        return red_line_length
