...
export default {
  data() {
    return {
      ...
      rtcOptions: {
        appId: "<YOUR APP ID>", // TODO: 替换成Agora APP ID
        channel: "xeducation_channel",
        token: null
      },
      activeName: 'note',
      noteName: 'first',
  ...
  methods: {
    ...
    joinRoom() {
      try {
        const options = this.rtcOptions
        this.rtcUID = await this.rtcClient.join(options.appId, options.channel, options.token, null)
        ...
        this.$toast.show('成功加入频道！')
      } catch (error) {
        this.$toast.show(`错误：无法加入频道，原因 ${error}`)
      }
    },
    setAdaptiveParameters(width, height, bitrate) {
      // 设置默认参数
      const bitrate = bitrate || 120
      const width = width || 400
      const height = heght || 400

      this.rtcClient.setLowStreamParameter({
        width, height, bitrate,
        framerate: 15 // 锁定帧率为15fps（减少资源占用）
      })

      // 开启视频双流模式
      this.rtcClient.enableDualStream().then(() => {
        this.$toast.show('视频双流模式开启成功！')
      }).catch(err => {
        this.$toast.show(`错误：无法开启视频双流模式，原因 ${error}`)
      })
    },
    changeRemoteSource(stream, streamType) {
      // 封装 SDK 的函数调用
      this.rtcClient.setRemoteVideoStreamType(stream, streamType)
    }
  }
}
</script>
...
.home-container {
  ...
  .left-container {
    ...
    float: left;
    // .left-top-container :hover div{
    //     display:block;
    //     background-color:red;
    // }
    // .left-top-container div{
    // }
    .left-middle-barrage {
      height: 120px;
      width: 70%;
