<template>
    <div class="all-follows">
        ...
        <div v-if="!loading">
            <div class="content" v-if="followList.length!==0">
                <div class="all-follows" v-for="(item,index) in followList" :key="index">
                    <div class="per-follow">
                        ...
                        <div class="btn-groups">
                            ...
                            <div class="support" @click="unsupport(item,index)"><p>已关注</p></div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="none" v-else>
                <p>暂未关注任何用户</p>
                <div class="go" @click="navigateTo()">去看看</div>
            </div>
        </div>
...
export default {
  data () {
    return {
      ...
      loading: true // 用骨架图替换loading图标加载
    }
  },
  methods: {
    navigateTo () {
    ...
    unsupport (item, index) {
      API.users.unSupport({userId: this.loginUser._id, supportUserId: item.supportUserDetail._id}, (responseData) => {
  ...
  created () {
    API.users.getSupportUserList({userId: this.loginUser._id}, (responseData) => { // 获取用户的关注列表
      if (responseData.code === 200) {
        console.log(responseData)
        this.followList = responseData.data
        this.loading = false
      } else {
        console.log(responseData)
      }
