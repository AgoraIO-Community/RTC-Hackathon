...
public class VideoActivity extends AppCompatActivity {
    ...
    private void joinChannel() {
        ...
        mRtcEngine.joinChannel(accessToken, MyApplication.CHANNEL_NAME_1, "Extra Optional Data", MyApplication.CLIENT_UID_1); // 未指定 uid，SDK 会自动分配一个。
    }
...
//        远端用户通过调用 addInjectStreamUrl 方法成功输入在线媒体流
    ...
    public void onScreenSharing(){
        ...
        try {
            ...
            for (int i = 0; i < count; i++) {
                View v = mRemoteContainer.getChildAt(i);
                if (v.getTag() instanceof Integer &&((int) v.getTag()) == MyApplication.CALL_UID ) {
                    view = v;
                }
            }
